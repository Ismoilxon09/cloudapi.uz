...
@section('title', __('auth.login.title') . ' — CloudAPI')
...
.auth-brand-mark svg { width: 100%; height: 100%; fill: currentColor; }
...
.forgot-link:hover { color: var(--text-strong); }
</style>
@endpush

@section('content')
<div class="auth-page">
  <!-- Top toolbar (lang + theme) -->
  ...
  <div class="auth-card">
    ...
    <form action="{{ route('login') }}" method="POST">
      ...
      <div class="field">
        <label class="label">{{ __('auth.login.email') }}</label>
        <input type="email" name="email" class="input" value="{{ old('email') }}" required autofocus autocomplete="email">
      </div>

      <div class="field">
        <label class="label">{{ __('auth.login.password') }}</label>
      ...
      <div class="form-row">
        ...
        <a href="{{ route('password.request') }}" class="forgot-link">{{ __('auth.login.forgot') }}</a>
      </div>
    ...
    <div class="oauth-divider">
      <span>yoki davom etish</span>
    </div>

    <div class="oauth-buttons">
      <a href="{{ route('telegram.login') }}" class="oauth-btn oauth-telegram" title="Telegram orqali kirish" aria-label="Telegram orqali kirish">
      ...
      <a href="{{ route('oauth.redirect', 'google') }}" class="oauth-btn oauth-google" title="Google orqali kirish" aria-label="Google orqali kirish">
      ...
      <a href="{{ route('oauth.redirect', 'github') }}" class="oauth-btn oauth-github" title="GitHub orqali kirish" aria-label="GitHub orqali kirish">
    ...
    <div class="auth-footer">
      ...
      <a href="{{ route('register') }}">{{ __('auth.login.sign_up') }}</a>
    </div>
  </div>
</div>
...
/* Provayder-specific (faqat hover background, icon o'z rangida turadi) */
.oauth-github:hover {
  background: #24292E;
  border-color: #24292E;
}
.oauth-github:hover svg {
  fill: white;
}
